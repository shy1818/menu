...
class Demo extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      useBodyScroll: false,// 是否使用body来做滚动容器
    };
  }
  ...
  componentDidUpdate() {
    // 状态变化后（是否使用body做容器）的时候，控制body的overflow属性
  ...
  render() {
    ...
    const row = (rowData, sectionID, rowID) => { // 每一行数据
      if (index < 0) {
        index = data.length - 1;
      }
      const obj = data[index--];// 每次执行的时候，obj都是data中的某一个数据，循环取值
      return (
        <div key={rowID}
          style={{
            padding: '0 15px',
